 ...
 * abstract base class TripleIndexFile.
 * It provides an insert/delete interface.
 */
public class TripleBTreeFile extends TripleIndexFile
	...
	private TripleBTreeHeaderPage headerPage;
	 ...
	 *          of this btree file.
	 */
	public TripleBTreeHeaderPage getHeaderPage() {
		return headerPage;
	}
	 ...
	 */
	public TripleBTreeFile(String filename)
			throws GetFileEntryException,
			PinPageException,
			ConstructPageException
	{
		...
		headerPage= new  TripleBTreeHeaderPage( headerPageId);
	 ...
	 */
	public TripleBTreeFile(String filename, int keytype,
			...
			AddFileEntryException
	{
		...
		{
			headerPage= new TripleBTreeHeaderPage();
			...
			FreePageException
	{
		...
		TripleBTSortedPage sortedPage;
		Page page=pinPage(pageno) ;
		sortedPage= new TripleBTSortedPage( page, headerPage.get_keyType());

		if (sortedPage.getType() == NodeType.INDEX) {
			TripleBTIndexPage indexPage= new TripleBTIndexPage( page, headerPage.get_keyType());
			TID      tid=new TID();
			PageId       childId;
			KeyDataEntry entry;
			...
			UnpinPageException
	{
		...
		TripleBTreeHeaderPage header;
		PageId old_data;


		header= new TripleBTreeHeaderPage( pinPage(headerPageId));
	 ...
	 *@param tid the rid of the record. Input parameter.
...

	{
		...
		if (TripleBT.getKeyLength(key) > headerPage.get_maxKeySize())
		...
		if (headerPage.get_rootId().pid == INVALID_PAGE) {
			...
			TripleBTLeafPage newRootPage;
			TID dummytid;

			newRootPage=new TripleBTLeafPage( headerPage.get_keyType());
		...
		if (newRootEntry != null)
		{
			...
			newRootPage = new TripleBTIndexPage(headerPage.get_keyType());
			newRootPageId=newRootPage.getCurPage();
...

	{
		...
		TripleBTSortedPage currentPage;
		Page page;
		KeyDataEntry upEntry;


		page=pinPage(currentPageId);
		currentPage=new TripleBTSortedPage(page, headerPage.get_keyType());
		...
		if(currentPage.getType() == NodeType.INDEX) {
			TripleBTIndexPage  currentIndexPage=new TripleBTIndexPage(page,
			...
			currentIndexPage= new  TripleBTIndexPage(pinPage(currentPageId),
					headerPage.get_keyType() );
			...
			if (currentIndexPage.available_space() >=
					TripleBT.getKeyDataLength( upEntry.key, NodeType.INDEX))
			{
			...
			if (TripleBT.keyCompare( upEntry.key, tmpEntry.key) >=0 )
			{
				...
				tmpEntry =
						TripleBT.getEntryFromBytes(currentIndexPage.getpage(),
		...
		{
			TripleBTLeafPage currentLeafPage =
					new TripleBTLeafPage(page, headerPage.get_keyType() );
			...
			if (currentLeafPage.available_space() >=
					TripleBT.getKeyDataLength(key, NodeType.LEAF) )
			{
				// no split has occurred
			...
			{
				TripleBTLeafPage rightPage;
				rightPage=new TripleBTLeafPage(rightPageId, headerPage.get_keyType());
				 ...
				 tmpEntry = currentLeafPage.getFirst(firstTid))
			{
				...
				newLeafPage.insertRecord( tmpEntry.key,
						((TripleLeafData)(tmpEntry.data)).getData());
			...
			if (TripleBT.keyCompare(key, undoEntry.key ) <  0) {
				//undo the final record
				if ( currentLeafPage.available_space() <
						newLeafPage.available_space()) {
					newLeafPage.insertRecord( undoEntry.key,
							((TripleLeafData)undoEntry.data).getData());
	...
	/*
	 * findRunStart.
	 * Status TripleBTreeFile::findRunStart (const void   lo_key,
	 *                                TID          *pstarttid)
	 *
	...
	TripleBTLeafPage findRunStart (KeyClass lo_key,
								   TID starttid)
			throws IOException,
			IteratorException,
			KeyNotMatchException,
			...
			UnpinPageException
	{
		TripleBTLeafPage  pageLeaf;
		TripleBTIndexPage pageIndex;
		Page page;
		TripleBTSortedPage  sortPage;
		PageId pageno;
		...
		page= pinPage(pageno);
		sortPage=new TripleBTSortedPage(page, headerPage.get_keyType());
		...
		while (sortPage.getType() == NodeType.INDEX) {
			pageIndex=new TripleBTIndexPage(page, headerPage.get_keyType());
			prevpageno = pageIndex.getPrevPage();
			curEntry= pageIndex.getFirst(starttid);
			while ( curEntry!=null && lo_key != null
					&& TripleBT.keyCompare(curEntry.key, lo_key) < 0) {
		...
		pageLeaf = new TripleBTLeafPage(page, headerPage.get_keyType() );
		...
		while (curEntry==null) {
			...
			pageno = nextpageno;
			pageLeaf=  new TripleBTLeafPage( pinPage(pageno), headerPage.get_keyType());
			curEntry=pageLeaf.getFirst(starttid);
		}
		...
		while (TripleBT.keyCompare(curEntry.key, lo_key) < 0) {
	...
	/*
	 *  Status TripleBTreeFile::NaiveDelete (const void *key, const TID tid)
	 *
			...
			IteratorException
	{
		...
		while ( true ) {
			...
			while ( entry == null) { // have to go right
				...
				leafPage=new TripleBTLeafPage(pinPage(nextpage),
						headerPage.get_keyType() );
				entry=leafPage.getFirst(new TID());
			}

			if ( TripleBT.keyCompare(key, entry.key) > 0 )
				break;
			...
			leafPage=new TripleBTLeafPage(pinPage(nextpage), headerPage.get_keyType());
			...
			IOException
	{
		...
		if (sortPage.getType()==NodeType.LEAF ) {
			...
			PageId nextpage;
			TripleBTLeafPage leafPage;
			leafPage=new TripleBTLeafPage(page, headerPage.get_keyType());
			...
			while((tmpEntry!=null) && (TripleBT.keyCompare(key,tmpEntry.key)>=0)) {
				...
				if ( leafPage.delEntry(new KeyDataEntry(key, tid)) ) {
					...
					if ( (4+leafPage.available_space()) <=
						...
						// get a sibling
						TripleBTIndexPage  parentPage;
						parentPage =
								new TripleBTIndexPage(pinPage(parentPageId),
										headerPage.get_keyType());

						PageId siblingPageId=new PageId();
						TripleBTLeafPage siblingPage;
						int direction;
						...
						siblingPage=new TripleBTLeafPage(pinPage(siblingPageId),
								headerPage.get_keyType());
						...
						if (siblingPage.redistribute( leafPage, parentPage,
							...
							TripleBTLeafPage leftChild, rightChild;
							if (direction==-1) {
							...
							if ( rightChild.getNextPage().pid != INVALID_PAGE) {
								TripleBTLeafPage nextLeafPage=new TripleBTLeafPage(
		...
		if (  sortPage.getType() == NodeType.INDEX ) {
			...
			indexPage=new TripleBTIndexPage(pinPage(currentPageId), headerPage.get_keyType());
			...
			if (indexPage.getCurPage().pid == headerPage.get_rootId().pid) {
				...
				if (indexPage.numberOfRecords() == 0) {
					TripleBTSortedPage childPage;
					childPage=new TripleBTSortedPage(indexPage.getPrevPage(),
							headerPage.get_keyType());
			...
			if ((4 /*sizeof slot*/ +indexPage.available_space()) <=
				...
				// get a sibling
				TripleBTIndexPage  parentPage;
				parentPage=new TripleBTIndexPage(pinPage(parentPageId),
						headerPage.get_keyType());

				PageId siblingPageId=new PageId();
				TripleBTIndexPage siblingPage;
				int direction;
				...
				siblingPage=new TripleBTIndexPage( pinPage(siblingPageId),
						headerPage.get_keyType());

				int pushKeySize=0;
				if (direction==1) {
					pushKeySize=TripleBT.getKeyLength
				...
				if (siblingPage.redistribute(indexPage,parentPage,
					...
					// merge the two children
					TripleBTIndexPage leftChild, rightChild;
					if (direction==-1) {
						leftChild = siblingPage;
	 ...
	 *@exception UnpinPageException error when unpin a page
	 */
	public TripleBTFileScan new_scan(KeyClass lo_key, KeyClass hi_key)
			throws IOException,
			KeyNotMatchException,
			IteratorException,
			ConstructPageException,
	...
	{
		TripleBTFileScan scan = new TripleBTFileScan();
		if ( headerPage.get_rootId().pid==INVALID_PAGE) {
			scan.leafPage=null;
			return scan;
		}
			...
			UnpinPageException
	{
		...
		if( trace!=null ) {
			...
			KeyClass key;
			KeyDataEntry entry;
			sortedPage=new TripleBTSortedPage( pinPage( id), headerPage.get_keyType());


			// Now print all the child nodes of the page.
			if( sortedPage.getType()==NodeType.INDEX) {
				TripleBTIndexPage indexPage=new TripleBTIndexPage(sortedPage,headerPage.get_keyType());
				...
				TripleBTLeafPage leafPage=new TripleBTLeafPage(sortedPage,headerPage.get_keyType());
