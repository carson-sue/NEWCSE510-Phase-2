 ...
 *
 */
public class QuadrupleBT  implements GlobalConst{
           ...
           ConvertException
    {
      ...
      try {
        ...
        if ( nodeType==NodeType.INDEX ) {
          ...
          n=8;
          QID qid=new QID();
          qid.slotNo =   Convert.getIntValue(offset+length-8, from);
          qid.pageNo =new PageId();
          qid.pageNo.pid= Convert.getIntValue(offset+length-4, from); 
           ...
           ConvertException
    {
      ...
      try{
        ...
        if( entry.data instanceof IndexData )
          n+=4;
        else if (entry.data instanceof QuadrupleLeafData )      
          n+=8;
        ...
        if ( entry.data instanceof IndexData ) {
          ...
          Convert.setIntValue( ((QuadrupleLeafData)entry.data).getData().slotNo,
                               m, data);
          Convert.setIntValue( ((QuadrupleLeafData)entry.data).getData().pageNo.pid,
                               m+4, data);
  ...
  public static void printPage(PageId pageno, int keyType)
    ...
    {
      QuadrupleBTSortedPage sortedPage=new QuadrupleBTSortedPage(pageno, keyType);
      int i;
      i=0;
      if ( sortedPage.getType()==NodeType.INDEX ) {
        QuadrupleBTIndexPage indexPage=new QuadrupleBTIndexPage((Page)sortedPage, keyType);
        System.out.println("");
        ...
        for(KeyDataEntry entry=indexPage.getFirst(qid); entry!=null; 
            entry=indexPage.getNext(qid)){
          if( keyType==AttrType.attrInteger) 
        ...
        for(KeyDataEntry entry=leafPage.getFirst(qid); entry!=null; 
            entry=leafPage.getNext(qid)){
          if( keyType==AttrType.attrInteger) 
            System.out.println(i+" (key, [pageNo, slotNo]):   ("+ 
                               (IntegerKey)entry.key+ ",  "+(QuadrupleLeafData)entry.data+ " )");
          if( keyType==AttrType.attrString) 
            System.out.println(i+" (key, [pageNo, slotNo]):   ("+ 
                               (StringKey)entry.key + ",  "+(QuadrupleLeafData)entry.data); 
  ...
  private static void _printTree(PageId currentPageId, String prefix, int i, 
           ...
           ReplacerException 
    {
      ...
      QuadrupleBTSortedPage sortedPage=new QuadrupleBTSortedPage(currentPageId, keyType);
      prefix=prefix+"       ";
      i++;
      if( sortedPage.getType()==NodeType.INDEX) {  
        QuadrupleBTIndexPage indexPage=new QuadrupleBTIndexPage((Page)sortedPage, keyType);
   ...
   *@exception ReplacerException  error from lower layer
   */
  public static void printAllLeafPages(QuadrupleBTreeHeaderPage header) 
    throws IOException, 
           ConstructPageException, 
           IteratorException,
           HashEntryNotFoundException,
