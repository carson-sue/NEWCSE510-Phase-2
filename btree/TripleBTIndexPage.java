...
/**
 * A TripleBTIndexPage is an index page on a label B+ tree.  It holds abstract 
 ...
 */
public class TripleBTIndexPage extends TripleBTSortedPage{


    /** pin the page with pageno, and get the corresponding TripleBTIndexPage,
     ...
     */
    public TripleBTIndexPage(PageId pageno, int keyType)
            throws IOException,
    ...
    /**associate the TripleBTIndexPage instance with the Page instance,
     * also it sets the type of node to be NodeType.INDEX.
     ...
     *@exception ConstructPageException error when BTIndexpage constructor
     */
    public TripleBTIndexPage(Page page, int keyType)
            throws IOException,
            ConstructPageException
    {
        super(page, keyType);
     ...
     *@exception ConstructPageException error when BTIndexpage constructor
     */
    public TripleBTIndexPage( int keyType)
            throws IOException,
            ConstructPageException
    {
        super(keyType);
            ...
            throws IndexFullDeleteException
    {
        ...
        try {
            ...
            while ( TripleBT.keyCompare(key, entry.key) > 0) {
                entry = getNext(tid );
                if (entry == null)
                    break;
            }
            ...
            else if (TripleBT.keyCompare(key, entry.key) != 0)
            ...
            throws IndexSearchException
    {
        ...
        try {
            ...
            for (i=getSlotCnt()-1; i >= 0; i--) {
                entry= TripleBT.getEntryFromBytes( getpage(),getSlotOffset(i),
            ...
            throws IteratorException
    {
        ...
        try {
            ...
            entry= TripleBT.getEntryFromBytes( getpage(),getSlotOffset(0),
                    getSlotLength(0),
            ...
            throws IndexFullDeleteException
    {
        ...
        try {
            ...
            for (i=getSlotCnt()-1; i >= 0; i--) {
                ...
                if (TripleBT.keyCompare(key, entry.key)>=0) {
                    if (i != 0) {
                        entry=TripleBT.getEntryFromBytes(getpage(), getSlotOffset(i-1),
     ...
     */
    boolean redistribute(TripleBTIndexPage indexPage, TripleBTIndexPage parentIndexPage,
        ...
        try {
            ...
            if (direction==-1) { // 'this' is the left sibling of indexPage
                if (( getSlotLength(getSlotCnt()-1) + available_space()) >
                    ...
                    lastEntry= TripleBT.getEntryFromBytes(getpage(), getSlotOffset(getSlotCnt()-1),
                ...
                if ( (getSlotLength(0) + available_space()) > ((MAX_SPACE-DPFIXED)/2) ) {
                    ...
                    firstEntry= TripleBT.getEntryFromBytes( getpage(),
                            getSlotOffset(0),
                            getSlotLength(0), keyType, NodeType.INDEX);
