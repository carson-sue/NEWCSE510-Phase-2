...
/**
 * QuadrupleBTFileScan implements a search/iterate interface to label B+ tree 
 * index files (class QuadrupleBTreeFile).  It derives from abstract base
 * class IndexFileScan.  
 */
public class QuadrupleBTFileScan  extends IndexFileScan
             implements  GlobalConst
{

  QuadrupleBTreeFile bfile; 
  String treeFilename;     // B+ tree we're scanning 
  QuadrupleBTLeafPage leafPage;   // leaf page containing current record
  QID curTid;       // position in current leaf; note: this is 
    ...
    try {
	 ...
	 if (nextpage.pid == INVALID_PAGE) {
	    ...
	    return null;
         ...
         leafPage=new QuadrupleBTLeafPage(nextpage, keyType);
	 	
	 entry=leafPage.getFirst(curTid);
      }

      if (endkey != null)  
        if ( QuadrupleBT.keyCompare(entry.key, endkey)  > 0) {
  ...
  public void delete_current() 
	...
	return;    
      ...
      entry=leafPage.getCurrent(curTid);  
      SystemDefs.JavabaseBM.unpinPage( leafPage.getCurPage(), false);
      bfile.Delete(entry.key, ((QuadrupleLeafData)entry.data).getData());
