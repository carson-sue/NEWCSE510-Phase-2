...
/** quadruplebtfile.java
 * This is the main definition of class BTreeFile, which derives from 
 * abstract base class QuadrupleIndexFile.
 * It provides an insert/delete interface.
 */
public class QuadrupleBTreeFile extends QuadrupleIndexFile
	...
	private QuadrupleBTreeHeaderPage headerPage;
	 ...
	 *          of this btree file.
	 */
	public QuadrupleBTreeHeaderPage getHeaderPage() {
		return headerPage;
	}
	 ...
	 */
	public QuadrupleBTreeFile(String filename)
			throws GetFileEntryException,
			PinPageException,
			ConstructPageException
	{
		...
		headerPage= new  QuadrupleBTreeHeaderPage( headerPageId);
		dbname = new String(filename);
		/*
		 *
		 * - headerPageId is the PageId of this LabelBTreeFile's header page;
	 ...
	 */
	public QuadrupleBTreeFile(String filename, int keytype,
			...
			AddFileEntryException
	{
		...
		{
			headerPage= new QuadrupleBTreeHeaderPage();
			...
			FreePageException
	{
		...
		QuadrupleBTSortedPage sortedPage;
		Page page=pinPage(pageno) ;
		sortedPage= new QuadrupleBTSortedPage( page, headerPage.get_keyType());

		if (sortedPage.getType() == NodeType.INDEX) {
			QuadrupleBTIndexPage indexPage= new QuadrupleBTIndexPage( page, headerPage.get_keyType());
			QID      qid=new QID();
			PageId       childId;
			KeyDataEntry entry;
			for (entry = indexPage.getFirst(qid);
				 entry!=null; entry = indexPage.getNext(qid))
			{
				childId = ((IndexData)(entry.data)).getData();
				_destroyFile(childId);
			}
		} else { // QuadrupleBTLeafPage
			...
			UnpinPageException
	{
		...
		QuadrupleBTreeHeaderPage header;
		PageId old_data;


		header= new QuadrupleBTreeHeaderPage( pinPage(headerPageId));
	...
	/** insert record with the given key and lid
	 ...
	 */
	public void insert(KeyClass key, QID qid)
...

	{
		...
		if (QuadrupleBT.getKeyLength(key) > headerPage.get_maxKeySize())
		...
		//    - we call _insert() to insert the pair (key, qid)


		if ( trace != null )
		{
			trace.writeBytes( "INSERT " + qid.pageNo + " "
					+ qid.slotNo + " " + key + lineSep);
		...
		if (headerPage.get_rootId().pid == INVALID_PAGE) {
			...
			QuadrupleBTLeafPage newRootPage;
			QID dummyqid;

			newRootPage=new QuadrupleBTLeafPage( headerPage.get_keyType());
			...
			newRootPage.insertRecord(key, qid);
		...
		newRootEntry= _insert(key, qid, headerPage.get_rootId());
		...
		{
			QuadrupleBTIndexPage newRootPage;
			PageId      newRootPageId;
			Object      newEntryKey;
			...
			newRootPage = new QuadrupleBTIndexPage(headerPage.get_keyType());
			newRootPageId=newRootPage.getCurPage();
	...
	private KeyDataEntry  _insert(KeyClass key, QID qid,
...

	{
		...
		QuadrupleBTSortedPage currentPage;
		Page page;
		KeyDataEntry upEntry;


		page=pinPage(currentPageId);
		currentPage=new QuadrupleBTSortedPage(page, headerPage.get_keyType());
		...
		if(currentPage.getType() == NodeType.INDEX) {
			QuadrupleBTIndexPage  currentIndexPage=new QuadrupleBTIndexPage(page,
			...
			upEntry= _insert(key, qid, nextPageId);
			...
			currentIndexPage= new  QuadrupleBTIndexPage(pinPage(currentPageId),
					headerPage.get_keyType() );
			...
			if (currentIndexPage.available_space() >=
					QuadrupleBT.getKeyDataLength( upEntry.key, NodeType.INDEX))
			{
			...
			if (QuadrupleBT.keyCompare( upEntry.key, tmpEntry.key) >=0 )
			{
				...
				tmpEntry =
						QuadrupleBT.getEntryFromBytes(currentIndexPage.getpage(),
		...
		{
			QuadrupleBTLeafPage currentLeafPage =
					new QuadrupleBTLeafPage(page, headerPage.get_keyType() );
			...
			if (currentLeafPage.available_space() >=
					QuadrupleBT.getKeyDataLength(key, NodeType.LEAF) )
			{
				// no split has occurred

				currentLeafPage.insertRecord(key, qid);
			...
			{
				QuadrupleBTLeafPage rightPage;
				rightPage=new QuadrupleBTLeafPage(rightPageId, headerPage.get_keyType());
				 ...
				 tmpEntry = currentLeafPage.getFirst(firstTid))
			{
				...
				newLeafPage.insertRecord( tmpEntry.key,
						((QuadrupleLeafData)(tmpEntry.data)).getData());
			...
			if (QuadrupleBT.keyCompare(key, undoEntry.key ) <  0) {
				//undo the final record
				if ( currentLeafPage.available_space() <
						newLeafPage.available_space()) {
					newLeafPage.insertRecord( undoEntry.key,
							((QuadrupleLeafData)undoEntry.data).getData());
			...
			if (QuadrupleBT.keyCompare(key,undoEntry.key ) >= 0)
			{
				...
				newLeafPage.insertRecord(key, qid);
	 ...
	 *@param qid the qid in pair <key, qid>. Input Parameter.
	 ...
	 *
	 */
	public boolean Delete(KeyClass key, QID qid)
	...
	/*
	 * findRunStart.
	 * Status QuadrupleBTreeFile::findRunStart (const void   lo_key,
	 *                                QID          *pstartqid)
	 *
	...
	QuadrupleBTLeafPage findRunStart (KeyClass lo_key,
								   QID startqid)
			throws IOException,
			IteratorException,
			KeyNotMatchException,
			...
			UnpinPageException
	{
		QuadrupleBTLeafPage  pageLeaf;
		QuadrupleBTIndexPage pageIndex;
		Page page;
		QuadrupleBTSortedPage  sortPage;
		PageId pageno;
		...
		page= pinPage(pageno);
		sortPage=new QuadrupleBTSortedPage(page, headerPage.get_keyType());
		...
		while (sortPage.getType() == NodeType.INDEX) {
			pageIndex=new QuadrupleBTIndexPage(page, headerPage.get_keyType());
			prevpageno = pageIndex.getPrevPage();
			curEntry= pageIndex.getFirst(startqid);
			while ( curEntry!=null && lo_key != null
					&& QuadrupleBT.keyCompare(curEntry.key, lo_key) < 0) {

				prevpageno = ((IndexData)curEntry.data).getData();
				curEntry=pageIndex.getNext(startqid);
			}
		...
		pageLeaf = new QuadrupleBTLeafPage(page, headerPage.get_keyType() );

		curEntry=pageLeaf.getFirst(startqid);
		while (curEntry==null) {
			...
			pageno = nextpageno;
			pageLeaf=  new QuadrupleBTLeafPage( pinPage(pageno), headerPage.get_keyType());
			curEntry=pageLeaf.getFirst(startqid);
		}

		// ASSERTIONS:
		// - curkey, curLid: contain the first record on the
		...
		while (QuadrupleBT.keyCompare(curEntry.key, lo_key) < 0) {
			curEntry= pageLeaf.getNext(startqid);
	...
	/*
	 *  Status QuadrupleBTreeFile::NaiveDelete (const void *key, const QID qid)
	 *
	 * Remove specified data entry (<key, qid>) from an index.
	 *
	 ...
	 * to find the one containing <key,qid>, which we then delete via
	 * BTLeafPage::delUserLid.
	 */

	private boolean NaiveDelete ( KeyClass key, QID qid)
			...
			IteratorException
	{
		...
		if ( trace!=null )
		{
			trace.writeBytes("DELETE " +qid.pageNo +" " + qid.slotNo + " "
					+ key +lineSep);
			trace.writeBytes( "DO"+lineSep);
			trace.writeBytes( "SEARCH" +lineSep);
			trace.flush();
		...
		leafPage=findRunStart(key, curTid);  // find first page,rid of key
		...
		while ( true ) {
			...
			while ( entry == null) { // have to go right
				...
				leafPage=new QuadrupleBTLeafPage(pinPage(nextpage),
						headerPage.get_keyType() );
				entry=leafPage.getFirst(new QID());
			}

			if ( QuadrupleBT.keyCompare(key, entry.key) > 0 )
				break;

			if( leafPage.delEntry(new KeyDataEntry(key, qid)) ==true) {
			...
			leafPage=new QuadrupleBTLeafPage(pinPage(nextpage), headerPage.get_keyType());
		 ...
		 * the specified <key,lid> data entry does not exist.
		 */
	...
	/*
	 * Status LabelBTreeFile::FullDelete (const void *key, const LID lid)
	 *
	 * Remove specified data entry (<key, lid>) from an index.
	 *
	...
	private boolean FullDelete (KeyClass key,  QID qid)
			...
			IteratorException
	{
		...
		try {
			...
			_Delete(key, qid, headerPage.get_rootId(), null);
			...
			IOException
	{
		...
		if (sortPage.getType()==NodeType.LEAF ) {
			...
			PageId nextpage;
			QuadrupleBTLeafPage leafPage;
			leafPage=new QuadrupleBTLeafPage(page, headerPage.get_keyType());
			...
			QID delTid;
			// for all records with key equal to 'key', delete it if its lid = 'lid'
			while((tmpEntry!=null) && (QuadrupleBT.keyCompare(key,tmpEntry.key)>=0)) {
				// WriteUpdateLog is done in the btleafpage level - to log the
				// deletion of the lid.

				if ( leafPage.delEntry(new KeyDataEntry(key, qid)) ) {
					// successfully found <key, lid> on this page and deleted it.
					...
					if ( (4+leafPage.available_space()) <=
						...
						// get a sibling
						QuadrupleBTIndexPage  parentPage;
						parentPage =
								new QuadrupleBTIndexPage(pinPage(parentPageId),
										headerPage.get_keyType());

						PageId siblingPageId=new PageId();
						QuadrupleBTLeafPage siblingPage;
						int direction;
						...
						siblingPage=new QuadrupleBTLeafPage(pinPage(siblingPageId),
								headerPage.get_keyType());
						...
						if (siblingPage.redistribute( leafPage, parentPage,
							...
							QuadrupleBTLeafPage leftChild, rightChild;
							if (direction==-1) {
							...
							QID firstTid=new QID(), insertTid;
							...
							if ( rightChild.getNextPage().pid != INVALID_PAGE) {
								QuadrupleBTLeafPage nextLeafPage=new QuadrupleBTLeafPage(
		...
		if (  sortPage.getType() == NodeType.INDEX ) {
			...
			KeyClass oldChildKey= _Delete(key, qid,  childPageId, currentPageId);
			...
			indexPage=new QuadrupleBTIndexPage(pinPage(currentPageId), headerPage.get_keyType());
			...
			PageId dummyPageId;
			KeyClass deletedKey = key;
			QID curTid=indexPage.deleteKey(oldChildKey);

			if (indexPage.getCurPage().pid == headerPage.get_rootId().pid) {
				// the index page is the root
				if (indexPage.numberOfRecords() == 0) {
					QuadrupleBTSortedPage childPage;
					childPage=new QuadrupleBTSortedPage(indexPage.getPrevPage(),
							headerPage.get_keyType());
			...
			if ((4 /*sizeof slot*/ +indexPage.available_space()) <=
				...
				// get a sibling
				QuadrupleBTIndexPage  parentPage;
				parentPage=new QuadrupleBTIndexPage(pinPage(parentPageId),
						headerPage.get_keyType());

				PageId siblingPageId=new PageId();
				QuadrupleBTIndexPage siblingPage;
				int direction;
				...
				siblingPage=new QuadrupleBTIndexPage( pinPage(siblingPageId),
						headerPage.get_keyType());

				int pushKeySize=0;
				if (direction==1) {
					pushKeySize=QuadrupleBT.getKeyLength
				...
				if (siblingPage.redistribute(indexPage,parentPage,
					...
					// merge the two children
					QuadrupleBTIndexPage leftChild, rightChild;
					if (direction==-1) {
						leftChild = siblingPage;
	 ...
	 *@exception UnpinPageException error when unpin a page
	 */
	public QuadrupleBTFileScan new_scan(KeyClass lo_key, KeyClass hi_key)
			throws IOException,
			KeyNotMatchException,
			IteratorException,
			ConstructPageException,
	...
	{
		QuadrupleBTFileScan scan = new QuadrupleBTFileScan();
		if ( headerPage.get_rootId().pid==INVALID_PAGE) {
			scan.leafPage=null;
			return scan;
		}
			...
			UnpinPageException
	{
		...
		if( trace!=null ) {
			...
			KeyClass key;
			KeyDataEntry entry;
			sortedPage=new QuadrupleBTSortedPage( pinPage( id), headerPage.get_keyType());


			// Now print all the child nodes of the page.
			if( sortedPage.getType()==NodeType.INDEX) {
				QuadrupleBTIndexPage indexPage=new QuadrupleBTIndexPage(sortedPage,headerPage.get_keyType());
				...
				QuadrupleBTLeafPage leafPage=new QuadrupleBTLeafPage(sortedPage,headerPage.get_keyType());
