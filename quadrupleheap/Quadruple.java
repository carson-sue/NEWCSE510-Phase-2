...
public class Quadruple implements GlobalConst{
 ...
 /** 
  ...
  private short [] fldOffset; 
   ...
   /**
    * Class constructor
    * Create a new Quadruple with length = max_size,Quadruple offset = 0.
    */
  ...
  public  Quadruple()
  {
       ...
       data = new byte[RDF_TRIPLE_SIZE];
       quadruple_offset = 0;
       quadruple_length = RDF_TRIPLE_SIZE;
  }
 ...
 * @throws IOException 
   ...
   public Quadruple(byte [] aquadruple, int offset,int length) throws IOException
   {
	   data = aquadruple;
 ...
 * @throws IOException 
    * 
    */
   public Quadruple(Quadruple aquadruple) throws IOException
   {
       data = aquadruple.getQuadrupleByteArray();
       quadruple_length = aquadruple.getLength();
       quadruple_offset = 0;
       setConfidence(aquadruple.getConfidence());
       setSubjectID(aquadruple.getSubjectID());
       setPredicateID(aquadruple.getPredicateID());
       setObjectID(aquadruple.getObjectID());
       Subject.writeToByteArray(data,0);
	   Predicate.writeToByteArray(data,8);
	   Object.writeToByteArray(data,16);
	   Convert.setDoubleValue(aquadruple.getConfidence(),24,data);
   ...
   public void quadrupleCopy(Quadruple fromQuadruple)
   {
	   try{
		...
		System.out.println("Error in quadruple copy");
	} 
   }
  ...
  {
	  	EID subjecqid = new EID();
	  	try {
			subjecqid.slotNo = Convert.getIntValue(0,data);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	  	try {
			subjecqid.pageNo = new PageId(Convert.getIntValue(4,data));
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
  ...
  {
	  EID objecqid = new EID();
	  	try {
			objecqid.slotNo = Convert.getIntValue(16,data);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	  	try {
			objecqid.pageNo = new PageId(Convert.getIntValue(20,data));
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
 ...
 public void quadrupleSet(byte [] record, int offset,int length)  
  {
      System.arraycopy(record, offset, data, 0, RDF_TRIPLE_SIZE);
      quadruple_offset = 0;
      quadruple_length = length;
  }
 ...
 throws IOException, InvalidTypeException, InvalidQuadrupleSizeException		
{
  ...
  for (i=1; i<numFlds; i++)
  {
   ...
   default:
    throw new InvalidTypeException (null, "Quadruple: Quadruple_TYPE_ERROR");
   }
