    ...
    */
			...
			Exception
	{
		...
		{// Start While01
			...
			{
				try{
					aQuadruple = currentDirPage.getRecord(currentDataPageRid);
				}
				...
				{
					aQuadruple = currentDataPage.returnRecord(rid);
...

	{
		...
		while(currentDirPageId.pid != INVALID_PAGE)
		{
			...
			{
				aQuadruple = currentDirPage.getRecord(rid);
				DataPageInfo dpinfo = new DataPageInfo(aQuadruple);
			...
			IOException
	{
		...
		{ //Start While01
						 ...
						 currentDirPage.nextRecord(currentDataPageRid))
			{
				...
				dpinfo = new DataPageInfo(aQuadruple);
		...
		aQuadruple = currentDirPage.returnRecord(currentDataPageRid);
		DataPageInfo dpinfo_ondirpage = new DataPageInfo(aQuadruple);
...

	{
		...
		aQuadruple = currentDirPage.returnRecord(currentDataPageRid);
		DataPageInfo pdpinfo = new DataPageInfo(aQuadruple);
	 ...
	 */
	public boolean updateRecord(QID rid, Quadruple newQuadruple)
			throws InvalidSlotNumberException,
			...
			Exception
	{
		...
		if(status != true) return status;	// record not found
		Quadruple aQuadruple = new Quadruple();
		aQuadruple = dataPage.returnRecord(rid);

		// Assume update a record with a record whose length is equal to
		// the original record

		if(newQuadruple.getLength() != aQuadruple.getLength())
		{
			unpinPage(currentDataPageId, false /*undirty*/);
			unpinPage(currentDirPageId, false /*undirty*/);
			...
			Exception
	{
		...
		Quadruple aQuadruple = new Quadruple();
		aQuadruple = dataPage.getRecord(rid);
